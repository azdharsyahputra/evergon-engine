...
const (
	hostsPath   = "/etc/hosts"
	beginMarker = "# BEGIN EVERGON TOOLS"
	endMarker   = "# END EVERGON TOOLS"
)
...
func ExplainHostsPermHint(err error) error {
	return fmt.Errorf("%w (need root to write /etc/hosts; run evergon with sudo or implement privileged helper)", err)
}
